program cgrid_shallow_water
  ...
  !PROGNOSTIC FIELDS:
  TYPE(rpe_var) :: h(0:nx,0:ny),u(0:nx,0:ny),v(0:nx,0:ny)
  !LOW PRECISION COPY OF PROGNOSTIC FIELDS FOR CO-DESIGNED SETUP:
  TYPE(rpe_var) :: hr(0:nx,0:ny),ur(0:nx,0:ny),vr(0:nx,0:ny)
  !WIND FORCING:
  TYPE(rpe_var) :: taux(0:ny),tauy(0:nx)
  !TIME INCREMENTS FOR ADAMS-BASHFORTH TIMESTEPPING SCHEME:
  TYPE(rpe_var) :: dh(0:nx,0:ny,0:nt),du(0:nx,0:ny,0:nt),dv(0:nx,0:ny,0:nt)
  !COEFFICIENTS OF ADAMS-BASHFORTH TIMESTEPPING SCHEME:
  TYPE(rpe_var) :: ab(nt)
  ! CORIOLIS PARAMETER AT U AND V GRID-POINTS RESPECTIVELY
  TYPE(rpe_var) :: fu(0:ny),fv(0:ny)

  TYPE(rpe_var) :: gp,f0,au,dx,dy,rdx,rdy,dt
  TYPE(rpe_var) :: slip,g,rho0,h0
...
! INCRESE PRECISION FOR GRID SPACING IN MODEL INITIALISATION:
  dx%sbits = 23
  dy%sbits = 23
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  do n=1,nstop
     !UPDATE THE REDUCED PRECISION COPIES OF THE PROGNOSTIC FIELDS TO USE THEM TO CALCULATE
     !THE RIGHT HAND SIDE OF THE EQUATIONS IN THE CO-DESIGNED MODEL SETUP
     DO k=0,ny
        DO j=0,nx
           ur(j,k) = u(j,k)
           vr(j,k) = v(j,k)
...
subroutine rhs(n,nx,ny,nt,u,du,v,dv,h,dh,gp,rdx,rdy,au,taux,tauy,fu,fv,&
  ...
  INTEGER :: n,nx,ny,nt
  TYPE(rpe_var) :: h(0:nx,0:ny),u(0:nx,0:ny),v(0:nx,0:ny)
  TYPE(rpe_var) :: taux(0:ny),tauy(0:nx),b(0:nx,0:ny)
  TYPE(rpe_var) :: dh(0:nx,0:ny,0:nt),du(0:nx,0:ny,0:nt),dv(0:nx,0:ny,0:nt)
  TYPE(rpe_var) :: ab(nt)
  TYPE(rpe_var) :: h0
  integer i,j,k
  character*5 num,crun
  TYPE(rpe_var) :: fu(0:ny),fv(0:ny)
  TYPE(rpe_var) :: zeta(0:nx,0:ny)
  TYPE(rpe_var) :: gp,f0,au,rdx,rdy,dt,slip
  TYPE(rpe_var) :: r0,r1,r2,r3,r4,r5
  logical :: lrestart
...
subroutine timeupdate(n,nx,ny,nt,u,du,v,dv,h,dh,slip,ndump,num,nwrite,dx,dy)
  ...
  if (mod(n,nwrite).eq.0) then
     ...
     print *,num	
     call write_hdata_file(h,dh,nt,nx,ny,dx,dy,'./Output/h.codesign.'//num)
...
subroutine initialise(nx,ny,nt,lrestart,au,h0,dt,f0,&
  ...
  character*5 crun
  TYPE(rpe_var) :: fu(0:ny),fv(0:ny)
  TYPE(rpe_var) :: vec(2+nt)
  TYPE(rpe_var) :: gp,pi,f0,beta,au,x0,y0,dx,dy,dt
  logical :: lrestart
